...
chrome.runtime.onMessage.addListener(async (message, sender, sendResponse) => {
  ...
  if (amazonURL.includes('amazon') && asinID) {
    ...
    modal.setAttribute(
      'style',
      'background: transparent;top: 0;right: 0;position: fixed;height:100vh;z-index:999;display:block;overflow:hidden;width:100%'
    );
    console.log(chrome.runtime.getURL('index.html'));
    modal.innerHTML = `<object type="text/html" data="${chrome.runtime.getURL(
      'index.html'
    )}" style="height:100vh;width:100%;position:absolute"></object>`;
    if (!document.body) {
